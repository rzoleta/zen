...
export default function WordDetailScreen() {
  ...
  const detail = useQuery(wordQueryOptions(wordId)).data;
  if (!detail)
    return (
      <Screen
  ...
  const due =
    card.state === State.New ? "Not introduced" : formatDueDate(dueDate);
  const dueRelative =
    card.state === State.New ? null : formatRelativeDueDate(dueDate);
  return (
    <Screen
      backgroundClassName={modalBackground}
      contentContainerClassName="pb-6"
